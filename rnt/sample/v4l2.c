...
uint32_t GetFPSParam(int fd, double fps, struct v4l2_frmivalenum *pfrmival)
{
    ...
    if(pfrmival->index){
    	...
    	*pfrmival = frmival[i];
    	printf( "fps:request[%.1f],but set to [%.1f]\n", fps, fpss[i]);
    }
    printf("-%s:\n", __func__);
...
int extra_cam_setting(int camfd)
{
	...
	fps = GetFPSParam(camfd, (double)g_fps, &frmival);
    frmival.pixel_format = v4l2_pix_fmt;
    frmival.width = win_width;
    frmival.height = win_height;
...
/* convert from 4:2:2 YUYV interlaced to RGB888 pixel
...
//TODO : Can this be optimized by SIMD, openMP, opencl???
void yuyv_to_rgb24 (int width, int height, unsigned char *src, unsigned char *dst)
{
	...
	//pr_debug(DSP_INFO,"%s: called!, width=%d, height=%d\n", __func__, width, height);
...
* Y0 U0  Y1 V0  Y2 U2  Y3 V2  Y4 U4
* Y0, Y1, Y2 ... can be extracted every two-byte in low byte
*/

void extractY(int width, int height, unsigned char *src, unsigned char *dst)
{
	unsigned char *s=src;
	for(int i = 0; i < height ; i++)
		for(int j = 0; j < width; j++){
			dst[j + i * width]=*src++;//Y is in low byte of every 2-byte
			++src;
		}
}
...
static void processFrame(const void *p, int size)
{
	...
	cvRectangle(Ycopy, cvPoint(sroi.x, sroi.y),
				cvPoint(sroi.x+sroi.width, sroi.y+sroi.height),
				CV_RGB(255,0,0), ROI_BORDERW,8, 0);
	...
	if( ut1 && (ut2 > ut1)){
	  ...
	  cvPrintf(Ycopy, tempbuf, cvPoint(20,50), cvFont(1.5,1.0), CV_RGB(0,0,255));
	}
	ut1=ut2;

	sprintf(tempbuf,"press 'q' to quit...");
	cvPrintf(framecopy, tempbuf, cvPoint(10,460),cvFont(1.5,1.0), CV_RGB(255,255, 255));
	cvPrintf(Ycopy, tempbuf, cvPoint(10,460),cvFont(1.5,1.0), CV_RGB(255,255, 255));

	//cvShowImage(windowname, framecopy);
	cvShowImage(windowname, Ycopy);

	if(framecopy)
	  cvReleaseImage(&framecopy);
	if(Ycopy)
		cvReleaseImage(&Ycopy);
}
